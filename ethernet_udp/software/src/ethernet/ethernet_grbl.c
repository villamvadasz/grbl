...
void do_serial_grbl(void) {
	...
	}
	{
		INT nBytes = 0;
		UINT txLen = 0;
		if (isCharOutBuffer_grbl() != -1) {
			nBytes = UDPIsPutReady(udpSkt);
			if (nBytes >= 64) {
				unsigned char data = 0;
				static unsigned char packetCounter = 0;
				uint32 i = 0;
		    	...
		    	nBytes = UDPPutArray( (UINT8*)&ToSendDataBuffer, i);
			    UDPFlush();	
			}
		} else {
