...
volatile uint8 do_nuts_bolts_delay1ms = 0;
uint8 nuts_bolts_delayCnt = 0;

void do_nuts_bolts(void) {
	{
		uint8 isr_tempVal = 0;
		lock_isr();
		if (do_nuts_bolts_delay1ms) {
			do_nuts_bolts_delay1ms = 0;
			isr_tempVal = 1;
		}
		unlock_isr();
		if (isr_tempVal) {
			if (nuts_bolts_delayCnt != 0) {
				nuts_bolts_delayCnt--;
			}
		}
	}
...
void setDelayRunning(uint16 delay) {
	nuts_bolts_delayCnt = delay;
}

uint8 isDelayRunning(void) {
	uint8 result = 0;
	if (nuts_bolts_delayCnt != 0) {
		result = 1;
	}
