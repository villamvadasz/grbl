...
void eep_manager_ReadAll(void) {
	...
	eep_manager_persistent_eeprom = read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 3);
	eep_manager_persistent_eeprom <<= 8;
	eep_manager_persistent_eeprom += read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 2);
	eep_manager_persistent_eeprom <<= 8;
	eep_manager_persistent_eeprom += read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 1);
	eep_manager_persistent_eeprom <<= 8;
	eep_manager_persistent_eeprom += read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 0);
	...
	eep_manager_persistent_eeprom_2 = read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 3);
	eep_manager_persistent_eeprom_2 <<= 8;
	eep_manager_persistent_eeprom_2 += read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 2);
	eep_manager_persistent_eeprom_2 <<= 8;
	eep_manager_persistent_eeprom_2 += read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 1);
	eep_manager_persistent_eeprom_2 <<= 8;
	eep_manager_persistent_eeprom_2 += read_eeprom(EEPMANAGER_ADDR_PERSISTENT + 0);
}
...
void eepManagerEmpty(void) {
	...
	write_eeprom(EEPMANAGER_ADDR_PERSISTENT,     eep_manager_persistent_config & 0xFF);
	write_eeprom(EEPMANAGER_ADDR_PERSISTENT + 1, (eep_manager_persistent_config >> 8) & 0xFF);
	write_eeprom(EEPMANAGER_ADDR_PERSISTENT + 2, (eep_manager_persistent_config >> 16) & 0xFF);
	write_eeprom(EEPMANAGER_ADDR_PERSISTENT + 3, (eep_manager_persistent_config >> 24) & 0xFF);
}
...
uint8 eepManagerReadVar(EepManager_Item item, uint32 size, void * data) {
	...
	if (data != NULL) {
		...
		if (size == eepManager_ItemTable[item].size) {
			uint32 i = 0;
			uint8 crc = 0;
			uint8 crcRead = read_eeprom(addr + size);
			for (i = 0; i < size; i++) {
...
uint8 eepManagerWriteVar_Asynch(EepManager_Item item, uint32 size, const void * data) {
	...
	switch (eepManagerWriteVar_AsynchSm) {
		...
		case 3 : {
			uint8 dataTemp = read_eeprom(addr + eepManagerWriteVar_AsynchSm_i);
			uint8 defaultDataTemp = ((unsigned char * )data)[eepManagerWriteVar_AsynchSm_i];
			if (dataTemp != defaultDataTemp) {
				write_eeprom(addr + eepManagerWriteVar_AsynchSm_i, defaultDataTemp);
			}
		...
		case 45 : {
			uint8 dataTemp = read_eeprom(addr + second_item + eepManagerWriteVar_AsynchSm_i);
			uint8 defaultDataTemp = ((unsigned char * )data)[eepManagerWriteVar_AsynchSm_i];
			if (dataTemp != defaultDataTemp) {
				write_eeprom(addr + second_item + eepManagerWriteVar_AsynchSm_i, defaultDataTemp);
			}
			eepManagerWriteVar_AsynchSm_i++;
			if (eepManagerWriteVar_AsynchSm_i >= size) {
		...
		case 46 : {
			if (eepManager_ItemTable[item].protection != EEP_NO_CRC) {
				write_eeprom(addr + second_item + size, crc);
			}
			eepManagerWriteVar_AsynchSm = 0;
			finished = 1;
			break;
