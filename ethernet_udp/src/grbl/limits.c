...
#ifdef HARD_LIMIT_POLLING
	#ifndef HARD_LIMIT_FORCE_STATE_CHECK
...
volatile uint8 do_limits_1ms = 0;
volatile uint8 do_limits_polling = 0;
...
void limits_init(void) {
	...
	limits_enable();
		...
		limits_get_state_current = limits_get_state();
		#ifdef HARD_LIMIT_POLLING
...
void do_limits(void) {
	{
		...
		unlock_isr();
		if (isr_tempVal) {
			limits_get_state_debug = limits_get_state();
			#ifdef HARD_LIMIT_POLLING
	...
	if (do_limits_polling) {
		...
		if (sys.state != STATE_ALARM) {
			if (!(system_get_exec_alarm())) {
					...
					#else
						isLimitActivated = limits_get_state_current;
					#endif
					limits_get_state_last = limits_get_state_current;
					if (isLimitActivated) {
						mc_stop();
		...
		switch (limit_go_home_state) {
			case LIMIT_GO_HOME_INIT : {
				...
				n_active_axis = 0;
					...
					#ifdef HOMING_IS_JUST_ZEROING_MACHINE_POS_X
						#define HOMING_FORCE_SET_ORIGIN_X
						if (cycle_mask_asynch & (1 << X_AXIS)) {
							limit_go_home_state = LIMIT_GO_HOME_FINISHED;
						}
					#endif
					#ifdef HOMING_IS_JUST_ZEROING_MACHINE_POS_Y
						#define HOMING_FORCE_SET_ORIGIN_Y
						if (cycle_mask_asynch & (1 << Y_AXIS)) {
							limit_go_home_state = LIMIT_GO_HOME_FINISHED;
						}
					#endif
					#ifdef HOMING_IS_JUST_ZEROING_MACHINE_POS_Z
						#define HOMING_FORCE_SET_ORIGIN_Z
			...
			case LIMIT_GO_HOME_2 : {
				...
				if ( (approach == 0) && (limit_flag_for_homeing_EXEC_CYCLE_STOP)) {
					...
					if (!approach && (limits_get_state() & cycle_mask_asynch)) {
