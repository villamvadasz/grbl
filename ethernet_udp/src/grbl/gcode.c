...
#define GCODE_POSITION_SAVE_TIME 5000
...
void gc_main(void) {
	static uint8 timerTriggered = 0;
	if (
		(plan_check_full_buffer()) || (protocol_get_serialCharReceived() != 0) || 
		...
		protocol_clear_serialCharReceived();
		//full
		timerTriggered = 0;
	} else {
		//empty
		if (timerTriggered == 0) {
			...
			timerTriggered = 1;
		} else if (timerTriggered == 1) {
			if (readTimer(&gcodePositionSave) == 0) {
				timerTriggered = 2;
				#ifdef ENABLE_POSITION_SAVE_INTO_EEPROM_AFTER_5_SEC
					{
						uint8 idx;
						for (idx=0; idx<N_AXIS; idx++) {
						...
						grbl_eeprom_trigger_items();
					}
				#endif
			}
		} else if (timerTriggered == 2) {
			timerTriggered = 3;
		}
	}
}
