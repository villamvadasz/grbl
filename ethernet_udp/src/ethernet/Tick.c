...
static volatile BYTE vTickReading_isr[6] __attribute__ ((aligned));
...
{
	lock_isr();
	vTickReading[0] = vTickReading_isr[0];
	vTickReading[1] = vTickReading_isr[1];
	vTickReading[2] = vTickReading_isr[2];
	vTickReading[3] = vTickReading_isr[3];
	vTickReading[4] = vTickReading_isr[4];
	vTickReading[5] = vTickReading_isr[5];
	unlock_isr();
}
...
void isr_ethernet_tick_1ms(void) {
	// Increment internal high tick counter
	//dwInternalTicks++;
	{
		vTickReading_isr[0]++;
		if (vTickReading_isr[0] == 0) {
			vTickReading_isr[1]++;
			if (vTickReading_isr[1] == 0) {
				vTickReading_isr[2]++;
				if (vTickReading_isr[2] == 0) {
					vTickReading_isr[3]++;
					if (vTickReading_isr[3] == 0) {
						vTickReading_isr[4]++;
						if (vTickReading_isr[4] == 0) {
							vTickReading_isr[5]++;
							if (vTickReading_isr[5] == 0) {
							}
						}
					}
				}
