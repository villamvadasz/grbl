						...
						#endif
						{
							...
							{ // skip eeprom address
								...
								if (NVMemWriteWord(ProgAddress, WrData) != 0) {
									...
									if(HexRecordSt.MaxAddress < (UINT32)(ProgAddress + HexRecordSt.RecDataLen))
									{
										HexRecordSt.MaxAddress = (UINT32)(ProgAddress + HexRecordSt.RecDataLen);
									}

									if(HexRecordSt.MinAddress > (UINT32)ProgAddress)
									{
										HexRecordSt.MinAddress = (UINT32)ProgAddress;
									}
								#endif
							}
						}
