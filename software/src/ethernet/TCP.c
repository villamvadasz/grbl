...
#define TCP_START_TIMEOUT_VAL   	((DWORD)1000)	// Timeout to retransmit unacked data
#define TCP_DELAYED_ACK_TIMEOUT		((DWORD)100)	// Timeout for delayed-acknowledgement algorithm
#define TCP_FIN_WAIT_2_TIMEOUT		((DWORD)5000)	// Timeout for FIN WAIT 2 state
#define TCP_KEEP_ALIVE_TIMEOUT		((DWORD)10000)	// Timeout for keep-alive messages when no traffic is sent
#define TCP_CLOSE_WAIT_TIMEOUT		((DWORD)200)	// Timeout for the CLOSE_WAIT state
...
#define TCP_AUTO_TRANSMIT_TIMEOUT_VAL	((DWORD)40)	// Timeout before automatically transmitting unflushed data
#define TCP_WINDOW_UPDATE_TIMEOUT_VAL	((DWORD)200)	// Timeout before automatically transmitting a window update due to a TCPGet() or TCPGetArray() function call

#define TCP_SYN_QUEUE_MAX_ENTRIES	(3u) 					// Number of TCP RX SYN packets to save if they cannot be serviced immediately
#define TCP_SYN_QUEUE_TIMEOUT		((DWORD)3000)	// Timeout for when SYN queue entries are deleted if unserviceable
...
volatile unsigned int do_tcp_1ms = 0;
...
void do_tcp(void) {
	if (do_tcp_1ms) {
		...
		{
			unsigned int x = 0;
			for (x = 0; x < TCP_SOCKET_COUNT; x++) {
				if (TCBStubs[x].eventTime != 0) {
					TCBStubs[x].eventTime--;
				}
				if (TCBStubs[x].eventTime2 != 0) {
					TCBStubs[x].eventTime2--;
				}
				if (TCBStubs[x].OverlappedTimers.delayedACKTime != 0) {
					TCBStubs[x].OverlappedTimers.delayedACKTime--;
				}
				//if (MyTCBStub.OverlappedTimers.closeWaitTime != 0) {//Overlapped this is the same as the previous
				//	MyTCBStub.OverlappedTimers.closeWaitTime--;
				//}
			}
			
			for(x = 0; x < TCP_SYN_QUEUE_MAX_ENTRIES; x++) {
				if (SYNQueue[x].wTimestamp != 0) {
					SYNQueue[x].wTimestamp --;
				}
			}
		}
	}
...
BOOL TCPPut(TCP_SOCKET hTCP, BYTE byte)
{
	...
	else if(!MyTCBStub.Flags.bTimer2Enabled)
	{
		...
		MyTCBStub.eventTime2 = TCP_AUTO_TRANSMIT_TIMEOUT_VAL;
	}
...
BOOL TCPGet(TCP_SOCKET hTCP, BYTE* byte)
{
	...
	{
		MyTCBStub.Flags.bTimer2Enabled = TRUE;
		MyTCBStub.eventTime2 = TCP_WINDOW_UPDATE_TIMEOUT_VAL;
	}
...
void TCPTick(void)
{
	...
	for(hTCP = 0; hTCP < TCP_SOCKET_COUNT; hTCP++)
	{
		...
		if(MyTCBStub.Flags.bDelayedACKTimerEnabled)
		{
			...
			if(MyTCBStub.OverlappedTimers.delayedACKTime == 0)
				vFlags = ACK;
		}
		...
		if(MyTCBStub.smState == TCP_CLOSE_WAIT)
		{
			...
			if(MyTCBStub.OverlappedTimers.closeWaitTime == 0)
			{
				vFlags = FIN | ACK;
				MyTCBStub.smState = TCP_LAST_ACK;
			}
		...
		if(!MyTCBStub.Flags.bTimerEnabled)
		{
				...
				if(MyTCBStub.smState == TCP_ESTABLISHED)
				{
					...
					MyTCBStub.eventTime = TCP_KEEP_ALIVE_TIMEOUT;
				}
			#endif
			continue;
		}
		...
		switch(MyTCBStub.smState)
		{
			...
			case TCP_GATEWAY_SEND_ARP:
				...
				MyTCBStub.eventTime2 = MyTCB.retryInterval;
...
static void SendTCP(BYTE vTCPFlags, BYTE vSendFlags)
{
	...
	if(len || (vTCPFlags & (SYN | FIN)))
	{
		...
		MyTCBStub.eventTime = MyTCB.retryInterval;
		MyTCBStub.Flags.bTimerEnabled = 1;
	}
...
static BOOL FindMatchingSocket(TCP_HEADER* h, NODE_INFO* remote)
{
	...
	#if TCP_SYN_QUEUE_MAX_ENTRIES
	{
		...
		for(wQueueInsertPos = 0; wQueueInsertPos < TCP_SYN_QUEUE_MAX_ENTRIES; wQueueInsertPos++)
		{
			...
			SYNQueue[wQueueInsertPos].wTimestamp = TCP_SYN_QUEUE_TIMEOUT;
			return FALSE;
		}
		...
		case TCP_CLOSING:
			...
			if(MyTCBStub.smState == TCP_FIN_WAIT_1)
			{
				...
				if((MyTCB.MySEQ == localAckNumber) && MyTCB.flags.bFINSent)
				{
					...
					MyTCBStub.eventTime = TCP_FIN_WAIT_2_TIMEOUT;
					MyTCBStub.Flags.bTimerEnabled = 1;
	...
	if(wSegmentLength)
	{
		...
		else
		{
			...
			if(!MyTCBStub.Flags.bDelayedACKTimerEnabled)
			{
				...
				MyTCBStub.OverlappedTimers.delayedACKTime = TCP_DELAYED_ACK_TIMEOUT;
			}
		}
	}
		...
		if(MyTCB.RemoteSEQ + 1 == localSeqNumber + (DWORD)wSegmentLength)
		{
			...
			switch(MyTCBStub.smState)
			{
				...
				case TCP_ESTABLISHED:
					...
					MyTCBStub.OverlappedTimers.closeWaitTime = TCP_CLOSE_WAIT_TIMEOUT;
					break;
