...
__attribute__(( weak )) void sntp_user_callout(uint32 dwSNTPSeconds) {}
...
void SNTPClient(void)
{
    ...
    case SM_UDP_RECV:
        ...
        if (!UDPIsGetReady(MySocket)) {
            if ((getGlobalTime()) - dwTimer > NTP_REPLY_TIMEOUT) {
        ...
        dwLastUpdateTick = getGlobalTime();
		...
		{
			extern void sntp_user_callout(uint32 dwSNTPSeconds);
			sntp_user_callout(dwSNTPSeconds);
		}
        break;

    case SM_SHORT_WAIT:
        // Attempt to requery the NTP server after a specified NTP_FAST_QUERY_INTERVAL time (ex: 8 seconds) has elapsed.
        if ((getGlobalTime() / 0x10000) - dwTimer > (NTP_FAST_QUERY_INTERVAL / 65536ull)) {
            SNTPState = SM_HOME;
            MySocket = INVALID_UDP_SOCKET;
        }
        break;
    ...
    case SM_WAIT:
        ...
        if (getGlobalTime() / 0x10000 - dwTimer > (NTP_QUERY_INTERVAL / 65536ull)) {
            SNTPState = SM_HOME;
            MySocket = INVALID_UDP_SOCKET;
        }
 ...
 ***************************************************************************/
uint32_t SNTPGetUTCSeconds_last(void)
{
