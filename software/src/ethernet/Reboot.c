...
#define REBOOT_PORT		8069	// UDP TFTP port
...
//		MySocket = UDPOpen(REBOOT_PORT, NULL, INVALID_UDP_PORT);
	...
	while (1) {
		unsigned char reboot_temp_buffer[64];
		INT avlBytes = 0;

		avlBytes = UDPIsGetReady(MySocket);
		...
		if(avlBytes) {
			INT nBytes = 0;
			nBytes = UDPGetArray((BYTE*)reboot_temp_buffer, avlBytes );
			if (nBytes >= 5) {
				INT x = 0;
				for (x = 0; x < (nBytes - 4); x++) {
					if (
						(reboot_temp_buffer[x + 0] == 'R') && 
						(reboot_temp_buffer[x + 1] == 'E') && 
						(reboot_temp_buffer[x + 2] == 'S') && 
						(reboot_temp_buffer[x + 3] == 'E') && 
						(reboot_temp_buffer[x + 4] == 'T')
					)
					{
						mal_reset();
					}
