...
Timer StackTsk_Announce_autonomous;
static BOOL StackTsk_once = FALSE;

void StackInit(void) {
	while (1) {
		unsigned int result = StackInit_Async();
		if (result != 0) {
			break;
		}
	}
 ...
 *
 ********************************************************************/
unsigned int StackInit_Async(void)
{
	unsigned int result = 0;
	static unsigned int StackInit_Async_sm = 0;

	switch (StackInit_Async_sm) {
		case 0 : {
			smStack = SM_STACK_IDLE;

			init_timer(&StackTsk_Announce_autonomous);
			...
			StackInit_Async_sm = 1;
			break;
		}
		case 1 : {
			unsigned int MACInit_Async_result = MACInit_Async();
			if (MACInit_Async_result != 0) {
				StackInit_Async_sm = 2;
			}
			break;
		}
		case 2 : {
...
{
	#if defined STACK_USE_ANNOUNCE_AUTONOMOUS
		{
			if (read_timer(&StackTsk_Announce_autonomous) == 0) {
				write_timer(&StackTsk_Announce_autonomous, STACK_USE_ANNOUNCE_AUTONOMOUS);
				AnnounceIP();
			}
		}
	#endif
