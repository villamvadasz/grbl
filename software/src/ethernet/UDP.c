...
volatile unsigned int do_udp_1ms = 0;
	...
	if (do_udp_1ms) {
		do_udp_1ms = 0;
		{
			...
			for ( ss = 0; ss < MAX_UDP_SOCKETS; ss++ ) {
				if (UDPSocketInfo[ss].eventTime != 0) {
					UDPSocketInfo[ss].eventTime--;
				}
			}
		}
	}
	...
	for ( ss = 0; ss < MAX_UDP_SOCKETS; ss++ )
	{
		...
		switch(UDPSocketInfo[ss].smState)
		{
			...
			case UDP_GATEWAY_SEND_ARP:
				...
				UDPSocketInfo[ss].eventTime = UDPSocketInfo[ss].retryInterval;
...
WORD UDPPutArray(BYTE *cData, WORD wDataLen)
{
	...
	if (MACIsAvailable() != FALSE) {
		...
		if (MACPutArray(cData, wDataLen) == 0) {
			result = 0;
		} else {
			wPutOffset += wDataLen;
			if (wPutOffset > UDPTxCount) {
