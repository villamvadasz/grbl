...
CETYPE FILEfind( FILEOBJ foDest, FILEOBJ foCompareTo, BYTE cmd, BYTE mode)
{
	...
	short int fileCompareLfnIndex = 0;
	short int fileFoundLfnIndex = 0;
	short int fileFoundMaxLfnIndex = 0;
	short int lfnCountIndex = 0;
	BOOL  lfnFirstCheck = FALSE;
	BOOL  foundSFN = FALSE;
	BOOL  foundLFN = FALSE;
	BOOL  fileFoundDotPosition = FALSE;
	BOOL  fileCompareDotPosition = FALSE;
	BYTE  lfnMaxSequenceNum = 0;
	BYTE  reminder = 0;
...
BYTE PopulateEntries(FILEOBJ fo, WORD *fHandle, BYTE mode)
{
    ...
    strncpy(dir->DIR_Name, fo->name, DIR_NAMESIZE);
    strncpy(dir->DIR_Extension, (fo->name) + DIR_NAMESIZE, DIR_EXTENSION);
...
CETYPE FILEerase( FILEOBJ fo, WORD *fHandle, BYTE EraseClusters)
{
	...
	while(numberOfFileEntries--)
	{
		...
		{
            /* 8.3 File Name - entry*/
            dir->DIR_Name[0] = (char)DIR_DEL; // mark as deleted
...
int FScopy (const char * fileNameSrc, const char * fileNameDst) {
	int result = -1;
	if ((fileNameSrc != NULL) && (fileNameDst != NULL)) {
		FSFILE * src = NULL;
		FSFILE * dst = NULL;
		src = FSfopen(fileNameSrc, "r");
		dst = FSfopen(fileNameDst, "w");
		if ((src != NULL) && (dst != NULL)) {
			// Buffer for reading image file data
			BYTE ReadBuffer[FSCOPY_BUFFER_SIZE];
			result = 0;
			while(FSfeof(src) != EOF) {
				size_t nRead = 0;     // Number of bytes remaining to decode
				size_t nWrite = 0;     // Number of bytes remaining to decode
				nRead = FSfread(&ReadBuffer[0], 1, FSCOPY_BUFFER_SIZE, src);
				if (nRead != 0) {
					nWrite = FSfwrite(&ReadBuffer[0], 1, nRead, dst);
					if (nWrite != nRead) {
						if ((FSerrno != CE_GOOD) && (FSerrno != CE_EOF)) {
							result = -1;
							break;
						}
					}
...
BYTE FormatFileName( const char* fileName, FILEOBJ fptr, BYTE mode)
{
    ...
    if((NAME_LFN_TYPE == fileNameType) || (supportLFN == TRUE))
	{
		#if defined(SUPPORT_LFN)  	
