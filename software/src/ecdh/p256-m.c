 ...
 */
unsigned int scalar_mult_state = 0;
uint32_t scalar_mult_s_odd[8];
uint32_t scalar_mult_py_neg[8];
uint32_t scalar_mult_py_use[8];
uint32_t scalar_mult_rz[8];
uint32_t scalar_mult_negate;
unsigned int scalar_mult_i = 0;
uint32_t scalar_mult_bit;

static unsigned int scalar_mult(uint32_t rx[8], uint32_t ry[8], const uint32_t px[8], const uint32_t py[8], const uint32_t s[8]) {
	unsigned int result = E_OK;
	switch (scalar_mult_state) {
		case 0 : {
			result = E_BUSY;
			/*
			 ...
			 */
			u256_sub(scalar_mult_s_odd, p256_n.m, s); /* no carry, result still in [1, n-1] */
		...
		case 1 : {
			result = E_BUSY;
			scalar_mult_negate = ~s[0] & 1;
			u256_cmov(scalar_mult_s_odd, s, 1 - scalar_mult_negate);
			scalar_mult_state = 2;
			break;
		}
		case 2 : {
			...
			u256_set32(scalar_mult_py_use, 0);
			scalar_mult_state = 3;
			break;
		}
		case 3 : {
			result = E_BUSY;
			m256_sub_p(scalar_mult_py_neg, scalar_mult_py_use, py);
			scalar_mult_state = 4;
			break;
		}
		case 4 : {
		...
		case 6 : {
			result = E_BUSY;
			m256_set32(scalar_mult_rz, 1, &p256_p);
			scalar_mult_state = 7;
			break;
		}
		case 7 : {
			result = E_BUSY;
			u256_cmov(ry, scalar_mult_py_neg, scalar_mult_negate);
			scalar_mult_state = 8;
			break;
		}
		case 8 : {
		...
		case 9 : {
			result = E_BUSY;
			if (scalar_mult_i > 0) {
				scalar_mult_state = 10;
			} else {
				scalar_mult_state = 14;
			}
			break;
		}
			case 10 : {
				result = E_BUSY;
				scalar_mult_bit = (scalar_mult_s_odd[scalar_mult_i / 32] >> scalar_mult_i % 32) & 1;

				/* set (px, py_use) = sbit(bit) P' = sbit(bit) * (-1)^negate P */
				u256_cmov(scalar_mult_py_use, py, scalar_mult_bit ^ scalar_mult_negate);
				scalar_mult_state = 11;
				break;
			}
			case 11 : {
				result = E_BUSY;
				u256_cmov(scalar_mult_py_use, scalar_mult_py_neg, (1 - scalar_mult_bit) ^ scalar_mult_negate);

				scalar_mult_state = 12;
				break;
			}
			case 12 : {
				result = E_BUSY;
				/* Update R = 2 * R +- P' */
				point_double(rx, ry, scalar_mult_rz);
				scalar_mult_state = 13;
				break;
			}
			case 13 : {
				result = E_BUSY;
				point_add(rx, ry, scalar_mult_rz, px, scalar_mult_py_use);
				scalar_mult_i--;
				scalar_mult_state = 9;
				break;
			}
		case 14 : {
			result = E_OK;
			point_to_affine(rx, ry, scalar_mult_rz);
			scalar_mult_state = 0;
			break;
		}
		default : {
 ...
 */
unsigned int scalar_gen_with_pub_state = 0;
int scalar_gen_with_pub_ret;
unsigned scalar_gen_with_pub_nb_tried = 0;

static int scalar_gen_with_pub(uint8_t sbytes[32], uint32_t s[8], uint32_t x[8], uint32_t y[8], int *ret) {
	int result = E_OK;

	switch (scalar_gen_with_pub_state) {
		case 0 : {
			result = E_BUSY;
			scalar_gen_with_pub_ret = 0;
			scalar_gen_with_pub_nb_tried = 0;
			
			scalar_gen_with_pub_state = 1;
			break;
		}
		case 1 : {
			result = E_BUSY;
			if (scalar_gen_with_pub_nb_tried++ >= 4) {
				scalar_gen_with_pub_ret = -1;
				scalar_gen_with_pub_state = 99;
			} else {
				scalar_gen_with_pub_ret = p256_generate_random(sbytes, 32);
				if (scalar_gen_with_pub_ret != 0) {
					scalar_gen_with_pub_ret = -1;
					scalar_gen_with_pub_state = 99;
				} else {
					scalar_gen_with_pub_state = 2;
				}
			}
			break;
		}
		case 2 : {
			...
			scalar_gen_with_pub_ret = scalar_from_bytes(s, sbytes);

			scalar_gen_with_pub_state = 3;
			break;
		}
		case 3 : {
			result = E_BUSY;
		...
		case 4 : {
			...
			if (scalar_mult(x, y, p256_gx, p256_gy, s) != E_BUSY) {
				scalar_gen_with_pub_state = 99;
			}
		...
		case 99 : {
			result = E_OK;
			*ret = scalar_gen_with_pub_ret;
			scalar_gen_with_pub_state = 0;
		}
		default : {
			scalar_gen_with_pub_state = 0;
			result = E_NOT_OK;
 ...
 */
uint8_t p256_gen_keypair_priv[32];
uint8_t p256_gen_keypair_pub[64];
int p256_gen_keypair_ret;
unsigned int p256_gen_keypair_state = 0;
uint32_t p256_gen_keypair_s[8];
uint32_t p256_gen_keypair_x[8]; 
uint32_t p256_gen_keypair_y[8];

int p256_gen_keypair_start(void) {
	unsigned int result = E_NOT_OK;
	if (p256_gen_keypair_state == 0) {
		memset(p256_gen_keypair_priv, 0xCC, sizeof(p256_gen_keypair_priv));
		memset(p256_gen_keypair_pub, 0xCC, sizeof(p256_gen_keypair_pub));
		memset(p256_gen_keypair_s, 0xCC, sizeof(p256_gen_keypair_s));
		memset(p256_gen_keypair_x, 0xCC, sizeof(p256_gen_keypair_x));
		memset(p256_gen_keypair_y, 0xCC, sizeof(p256_gen_keypair_y));

		p256_gen_keypair_ret = 0;

		p256_gen_keypair_state = 1;
		result = E_OK;
	} else {
		result = E_NOT_OK;
	}
...
int p256_gen_keypair_update(void) {
	...
	switch (p256_gen_keypair_state) {
		...
		case 1 : {
			result = E_BUSY;
			if (scalar_gen_with_pub(p256_gen_keypair_priv, p256_gen_keypair_s, p256_gen_keypair_x, p256_gen_keypair_y, &p256_gen_keypair_ret) != E_BUSY) {
				p256_gen_keypair_state = 20;
			}
			break;
		}
		case 20 : {
			...
			zeroize(p256_gen_keypair_s, sizeof p256_gen_keypair_s);
			if (p256_gen_keypair_ret != 0) {
				p256_gen_keypair_ret = P256_RANDOM_FAILED;
				p256_gen_keypair_state = 99;
			} else {
				p256_gen_keypair_state = 30;
			}
		...
		case 30 : {
			...
			point_to_bytes(p256_gen_keypair_pub, p256_gen_keypair_x, p256_gen_keypair_y);
			p256_gen_keypair_ret = 0;
		...
		case 99 : {
			result = E_OK;
			p256_gen_keypair_state = 100;
		}
		default : {
			p256_gen_keypair_state = 0;
			result = E_NOT_OK;
			break;
		}
	}
...
int p256_gen_keypair_stop(uint8_t priv[32], uint8_t pub[64], int *ret) {
	unsigned int result = E_NOT_OK;
	if (ret != NULL) {
		if (p256_gen_keypair_state == 100) {
			memcpy(priv, p256_gen_keypair_priv, sizeof(p256_gen_keypair_priv));
			memcpy(pub, p256_gen_keypair_pub, sizeof(p256_gen_keypair_pub));
			*ret = p256_gen_keypair_ret;
			result = E_OK;
		} else {
			*ret = P256_INVALID_STATE;
			result = E_NOT_OK;
		}
	}
	return result;
...
int p256_gen_keypair(uint8_t priv[32], uint8_t pub[64]) {
	int ret = P256_INVALID_STATE;
	if (p256_gen_keypair_start() == E_OK) {
		while (p256_gen_keypair_update() == E_BUSY);
		if (p256_gen_keypair_stop(priv, pub, &ret) == E_OK) {
		}
	}
	return ret;
 ...
 */
uint8_t p256_ecdh_shared_secret_secret[32];
uint8_t p256_ecdh_shared_secret_priv[32];
uint8_t p256_ecdh_shared_secret_peer[64];
int p256_ecdh_shared_secret_ret;
unsigned int p256_ecdh_shared_secret_state = 0;
uint32_t p256_ecdh_shared_secret_s[8];
uint32_t p256_ecdh_shared_secret_px[8];
uint32_t p256_ecdh_shared_secret_py[8];
uint32_t p256_ecdh_shared_secret_x[8];
uint32_t p256_ecdh_shared_secret_y[8];

unsigned int p256_ecdh_shared_secret_start(const uint8_t priv[32], const uint8_t peer[64]) {
	unsigned int result = E_NOT_OK;
	if (p256_ecdh_shared_secret_state == 0) {
		memset(p256_ecdh_shared_secret_secret, 0xCC, sizeof(p256_ecdh_shared_secret_secret));
		//memset(p256_ecdh_shared_secret_priv, 0xCC, 32);
		//memset(p256_ecdh_shared_secret_peer, 0xCC, 64);
		p256_ecdh_shared_secret_ret = 0;
		memset(p256_ecdh_shared_secret_s, 0xCC, sizeof(p256_ecdh_shared_secret_s));
		memset(p256_ecdh_shared_secret_px, 0xCC, sizeof(p256_ecdh_shared_secret_px));
		memset(p256_ecdh_shared_secret_py, 0xCC, sizeof(p256_ecdh_shared_secret_py));
		memset(p256_ecdh_shared_secret_x, 0xCC, sizeof(p256_ecdh_shared_secret_x));
		memset(p256_ecdh_shared_secret_y, 0xCC, sizeof(p256_ecdh_shared_secret_y));

		memcpy(p256_ecdh_shared_secret_priv, priv, sizeof(p256_ecdh_shared_secret_priv));
		memcpy(p256_ecdh_shared_secret_peer, peer, sizeof(p256_ecdh_shared_secret_peer));
		p256_ecdh_shared_secret_state = 1;
		result = E_OK;
	} else {
		result = E_NOT_OK;
	}
...
unsigned int p256_ecdh_shared_secret_update(void) {
	unsigned int result = E_NOT_OK;
	switch (p256_ecdh_shared_secret_state) {
		case 0 : {
			result = E_NOT_OK;
			break;
		}
		case 1 : {
			...
			p256_ecdh_shared_secret_ret = scalar_from_bytes(p256_ecdh_shared_secret_s, p256_ecdh_shared_secret_priv);
			if (p256_ecdh_shared_secret_ret != 0) {
				p256_ecdh_shared_secret_ret = P256_INVALID_PRIVKEY;
				p256_ecdh_shared_secret_state = 99;
			}
			p256_ecdh_shared_secret_state = 20;
			break;
		}
		case 20 : {
			result = E_BUSY;

			p256_ecdh_shared_secret_ret = point_from_bytes(p256_ecdh_shared_secret_px, p256_ecdh_shared_secret_py, p256_ecdh_shared_secret_peer);
			if (p256_ecdh_shared_secret_ret != 0) {
				p256_ecdh_shared_secret_ret = P256_INVALID_PUBKEY;
				p256_ecdh_shared_secret_state = 99;
			}

			p256_ecdh_shared_secret_state = 30;
		...
		case 30 : {
			...
			while (E_BUSY == scalar_mult(p256_ecdh_shared_secret_x, p256_ecdh_shared_secret_y, p256_ecdh_shared_secret_px, p256_ecdh_shared_secret_py, p256_ecdh_shared_secret_s));

			p256_ecdh_shared_secret_state = 40;
		...
		case 40 : {
			...
			m256_to_bytes(p256_ecdh_shared_secret_secret, p256_ecdh_shared_secret_x, &p256_p);
			p256_ecdh_shared_secret_state = 99;
		...
		case 99 : {
			result = E_OK;
			zeroize(p256_ecdh_shared_secret_s, sizeof p256_ecdh_shared_secret_s);
			p256_ecdh_shared_secret_state = 100;
		}
		default : {
			p256_ecdh_shared_secret_state = 0;
			result = E_NOT_OK;
			break;
		}
	}
...
unsigned int p256_ecdh_shared_secret_stop(uint8_t secret[32], int *ret) {
	unsigned int result = E_NOT_OK;
	if (ret != NULL) {
		if (p256_ecdh_shared_secret_state == 100) {
			memcpy(secret, p256_ecdh_shared_secret_secret, sizeof(p256_ecdh_shared_secret_secret));
			*ret = p256_ecdh_shared_secret_ret;
			result = E_OK;
		} else {
			*ret = P256_INVALID_STATE;
			result = E_NOT_OK;
...
int p256_ecdh_shared_secret(uint8_t secret[32], const uint8_t priv[32], const uint8_t peer[64]) {
	int ret = P256_INVALID_STATE;
	if (p256_ecdh_shared_secret_start(priv, peer) == E_OK) {
		while (p256_ecdh_shared_secret_update() == E_BUSY);
		if (p256_ecdh_shared_secret_stop(secret, &ret) == E_OK) {
		}
	}
	return ret;
}
 ...
 * ECDSA sign
 */
int p256_ecdsa_sign_state = 0;
uint8_t p256_ecdsa_sign_sig[64];
uint8_t p256_ecdsa_sign_priv[32];
const uint8_t *p256_ecdsa_sign_hash;
size_t p256_ecdsa_sign_hlen;
int p256_ecdsa_sign_ret;
uint32_t p256_ecdsa_sign_xr[8];
uint32_t p256_ecdsa_sign_k[8];
uint32_t p256_ecdsa_sign_t3[8]; 
uint32_t p256_ecdsa_sign_t4[8];
uint8_t *p256_ecdsa_sign_kb;

int p256_ecdsa_sign_start(const uint8_t priv[32], const uint8_t *hash, size_t hlen) {
	unsigned int result = E_NOT_OK;
	if (p256_ecdsa_sign_state == 0) {
		p256_ecdsa_sign_ret = 0;

		memset(p256_ecdsa_sign_sig, 0xCC, sizeof(p256_ecdsa_sign_sig));
		memset(p256_ecdsa_sign_xr, 0xCC, sizeof(p256_ecdsa_sign_xr));
		memset(p256_ecdsa_sign_k, 0xCC, sizeof(p256_ecdsa_sign_k));
		memset(p256_ecdsa_sign_t3, 0xCC, sizeof(p256_ecdsa_sign_t3));
		memset(p256_ecdsa_sign_t4, 0xCC, sizeof(p256_ecdsa_sign_t4));

		memcpy(p256_ecdsa_sign_priv, priv, sizeof(p256_ecdsa_sign_priv));
		p256_ecdsa_sign_hash = hash;
		p256_ecdsa_sign_hlen = hlen;

		p256_ecdsa_sign_state = 1;
		result = E_OK;
	} else {
		result = E_NOT_OK;
	}
...
int p256_ecdsa_sign_update(void) {
	...
	switch (p256_ecdsa_sign_state) {
		...
		case 1 : {
			...
			p256_ecdsa_sign_kb = (uint8_t *) p256_ecdsa_sign_t4;

			p256_ecdsa_sign_state = 20;
			break;
		}
		case 20 : {
			result = E_BUSY;
			...
			/* xr = x_coord(k * G) */
			if (scalar_gen_with_pub(p256_ecdsa_sign_kb, p256_ecdsa_sign_k, p256_ecdsa_sign_xr, p256_ecdsa_sign_t3, &p256_ecdsa_sign_ret) != E_BUSY) {
				if (p256_ecdsa_sign_ret != 0) {
					p256_ecdsa_sign_ret = P256_RANDOM_FAILED;
					p256_ecdsa_sign_state = 99;
				} else {
					p256_ecdsa_sign_state = 25;
				}
			}
		...
		case 25 : {
			...
			m256_prep(p256_ecdsa_sign_k, &p256_n);

			p256_ecdsa_sign_state = 30;
			break;
		}
		case 30 : {
			result = E_BUSY;
			...
			m256_done(p256_ecdsa_sign_xr, &p256_p);

			p256_ecdsa_sign_state = 35;
		...
		case 35 : {
			...
			ecdsa_m256_mod_n(p256_ecdsa_sign_xr);    /* xr = int(xr) mod n */

			p256_ecdsa_sign_state = 40;
		...
		case 40 : {
			...
			if (u256_diff0(p256_ecdsa_sign_xr) == 0) {
				p256_ecdsa_sign_ret = P256_RANDOM_FAILED;
				p256_ecdsa_sign_state = 99;
			} else {
				p256_ecdsa_sign_state = 45;
			}
		...
		case 45 : {
			...
			u256_to_bytes(p256_ecdsa_sign_sig, p256_ecdsa_sign_xr);

			p256_ecdsa_sign_state = 50;
		...
		case 50 : {
			...
			m256_prep(p256_ecdsa_sign_xr, &p256_n);

			/* 4. Skipped - we take the hash as an input, not the message */

			p256_ecdsa_sign_state = 55;
		...
		case 55 : {
			...
			ecdsa_m256_from_hash(p256_ecdsa_sign_t3, p256_ecdsa_sign_hash, p256_ecdsa_sign_hlen);   /* t3 = e */

			p256_ecdsa_sign_state = 60;
		...
		case 60 : {
			...
			p256_ecdsa_sign_ret = scalar_from_bytes(p256_ecdsa_sign_t4, p256_ecdsa_sign_priv);   /* t4 = dU (integer domain) */
			p256_ecdsa_sign_state = 65;
		...
		case 65 : {
			...
			if (p256_ecdsa_sign_ret != 0) {
				p256_ecdsa_sign_ret = P256_INVALID_PRIVKEY;
				p256_ecdsa_sign_state = 99;
			} else {
				m256_prep(p256_ecdsa_sign_t4, &p256_n);         /* t4 = dU (Montgomery domain) */
				p256_ecdsa_sign_state = 70;
			}
		...
		case 70 : {
			...
			m256_inv(p256_ecdsa_sign_k, p256_ecdsa_sign_k, &p256_n);        /* k^-1 */
			p256_ecdsa_sign_state = 75;
		...
		case 75 : {
			...
			m256_mul(p256_ecdsa_sign_t4, p256_ecdsa_sign_xr, p256_ecdsa_sign_t4, &p256_n);  /* t4 = r * dU */
			p256_ecdsa_sign_state = 80;
		...
		case 80 : {
			...
			m256_add(p256_ecdsa_sign_t4, p256_ecdsa_sign_t3, p256_ecdsa_sign_t4, &p256_n);  /* t4 = e + r * dU */
			p256_ecdsa_sign_state = 85;
		...
		case 85 : {
			...
			m256_mul(p256_ecdsa_sign_t4, p256_ecdsa_sign_k, p256_ecdsa_sign_t4, &p256_n);   /* t4 = s = k^-1 * (e + r * dU) */
			p256_ecdsa_sign_state = 90;
		...
		case 90 : {
			...
			zeroize(p256_ecdsa_sign_k, sizeof p256_ecdsa_sign_k);

			p256_ecdsa_sign_state = 95;
		...
		case 95 : {
			...
			if (u256_diff0(p256_ecdsa_sign_t4) == 0) {
				/* undo early output of r */
				u256_to_bytes(p256_ecdsa_sign_sig, p256_ecdsa_sign_t4);
				p256_ecdsa_sign_ret = P256_RANDOM_FAILED;
				p256_ecdsa_sign_state = 99;
			}

			p256_ecdsa_sign_state = 96;
		...
		case 96 : {
			...
			m256_to_bytes(p256_ecdsa_sign_sig + 32, p256_ecdsa_sign_t4, &p256_n);

			p256_ecdsa_sign_ret = P256_SUCCESS;
			p256_ecdsa_sign_state = 99;
		...
		case 99 : {
			...
			p256_ecdsa_sign_state = 100;
			break;
		}

		default : {
			p256_ecdsa_sign_state = 0;
			result = E_NOT_OK;
			break;
		}
	}
...
int p256_ecdsa_sign_stop(uint8_t sig[64], int *ret) {
	unsigned int result = E_NOT_OK;
	if (ret != NULL) {
		if (p256_ecdsa_sign_state == 100) {
			memcpy(sig, p256_ecdsa_sign_sig, 64);
			*ret = p256_ecdsa_sign_ret;
			result = E_OK;
		} else {
			*ret = P256_INVALID_STATE;
			result = E_NOT_OK;
...
int p256_ecdsa_sign(uint8_t sig[64], const uint8_t priv[32], const uint8_t *hash, size_t hlen) {
	int ret = P256_INVALID_STATE;
	if (p256_ecdsa_sign_start(priv, hash, hlen) == E_OK) {
		while (p256_ecdsa_sign_update() == E_BUSY);
		if (p256_ecdsa_sign_stop(sig, &ret) == E_OK) {
		}
	}
	return ret;
}
 ...
 * ECDSA verify
 */
int p256_ecdsa_verify_state = 0;
uint8_t p256_ecdsa_verify_sig[64];
uint8_t p256_ecdsa_verify_pub[64]; 
uint8_t *p256_ecdsa_verify_hash;
size_t p256_ecdsa_verify_hlen;
int p256_ecdsa_verify_ret;
uint32_t p256_ecdsa_verify_r[8]; 
uint32_t p256_ecdsa_verify_s[8];
uint32_t p256_ecdsa_verify_e[8];
uint32_t p256_ecdsa_verify_u1[8];
uint32_t p256_ecdsa_verify_u2[8];
uint32_t p256_ecdsa_verify_px[8]; 
uint32_t p256_ecdsa_verify_py[8];

int p256_ecdsa_verify_start(const uint8_t sig[64], const uint8_t pub[64], const uint8_t *hash, size_t hlen) {
	unsigned int result = E_NOT_OK;
	if (p256_ecdsa_verify_state == 0) {
		p256_ecdsa_verify_ret = 0;

		memset(p256_ecdsa_verify_r, 0xCC, sizeof(p256_ecdsa_verify_r));
		memset(p256_ecdsa_verify_s, 0xCC, sizeof(p256_ecdsa_verify_s));
		memset(p256_ecdsa_verify_e, 0xCC, sizeof(p256_ecdsa_verify_e));
		memset(p256_ecdsa_verify_u1, 0xCC, sizeof(p256_ecdsa_verify_u1));
		memset(p256_ecdsa_verify_u2, 0xCC, sizeof(p256_ecdsa_verify_u2));
		memset(p256_ecdsa_verify_px, 0xCC, sizeof(p256_ecdsa_verify_px));
		memset(p256_ecdsa_verify_py, 0xCC, sizeof(p256_ecdsa_verify_py));

		memcpy(p256_ecdsa_verify_sig, sig, sizeof(p256_ecdsa_verify_sig));
		memcpy(p256_ecdsa_verify_pub, pub, sizeof(p256_ecdsa_verify_pub));
		p256_ecdsa_verify_hash = (uint8_t *)hash;
		p256_ecdsa_verify_hlen = hlen;

		p256_ecdsa_verify_state = 1;
		result = E_OK;
	} else {
		result = E_NOT_OK;
	}
...
int p256_ecdsa_verify_update(void) {
	...
	unsigned int result = E_NOT_OK;
	switch (p256_ecdsa_verify_state) {
		case 0 : {
			result = E_NOT_OK;
			break;
		}
		case 1 : {
			...
			p256_ecdsa_verify_ret = scalar_from_bytes(p256_ecdsa_verify_r, p256_ecdsa_verify_sig);
			if (p256_ecdsa_verify_ret != 0) {
				p256_ecdsa_verify_ret = P256_INVALID_SIGNATURE;
				p256_ecdsa_verify_state = 99;
			} else {
				p256_ecdsa_verify_state = 2;
			}
		}
		case 2 : {
			result = E_BUSY;

			p256_ecdsa_verify_ret = scalar_from_bytes(p256_ecdsa_verify_s, p256_ecdsa_verify_sig + 32);
			if (p256_ecdsa_verify_ret != 0) {
				p256_ecdsa_verify_ret = P256_INVALID_SIGNATURE;
				p256_ecdsa_verify_state = 99;
			} else {
				p256_ecdsa_verify_state = 3;
			}
			break;
		}
		case 3 : {
			...
			ecdsa_m256_from_hash(p256_ecdsa_verify_e, p256_ecdsa_verify_hash, p256_ecdsa_verify_hlen);

			p256_ecdsa_verify_state = 4;
			break;
		}
		case 4 : {
			result = E_BUSY;
			...
			m256_prep(p256_ecdsa_verify_s, &p256_n);           /* s in Montgomery domain */

			p256_ecdsa_verify_state = 5;
			break;
		}
		case 5 : {
			result = E_BUSY;

			m256_inv(p256_ecdsa_verify_s, p256_ecdsa_verify_s, &p256_n);         /* s = s^-1 mod n */

			p256_ecdsa_verify_state = 6;
			break;
		}
		case 6 : {
			result = E_BUSY;

			m256_mul(p256_ecdsa_verify_u1, p256_ecdsa_verify_e, p256_ecdsa_verify_s, &p256_n);     /* u1 = e * s^-1 mod n */

			p256_ecdsa_verify_state = 7;
			break;
		}
		case 7 : {
			result = E_BUSY;

			m256_done(p256_ecdsa_verify_u1, &p256_n);          /* u1 out of Montgomery domain */

			p256_ecdsa_verify_state = 8;
			break;
		}
		case 8 : {
			result = E_BUSY;

			u256_cmov(p256_ecdsa_verify_u2, p256_ecdsa_verify_r, 1);

			p256_ecdsa_verify_state = 9;
			break;
		}
		case 9 : {
			result = E_BUSY;

			m256_prep(p256_ecdsa_verify_u2, &p256_n);          /* r in Montgomery domain */

			p256_ecdsa_verify_state = 10;
			break;
		}
		case 10 : {
			result = E_BUSY;

			m256_mul(p256_ecdsa_verify_u2, p256_ecdsa_verify_u2, p256_ecdsa_verify_s, &p256_n);    /* u2 = r * s^-1 mod n */

			p256_ecdsa_verify_state = 11;
			break;
		}
		case 11 : {
			result = E_BUSY;

			m256_done(p256_ecdsa_verify_u2, &p256_n);          /* u2 out of Montgomery domain */

			p256_ecdsa_verify_state = 12;
			break;
		}
		case 12 : {
			result = E_BUSY;
			...
			p256_ecdsa_verify_ret = point_from_bytes(p256_ecdsa_verify_px, p256_ecdsa_verify_py, p256_ecdsa_verify_pub);
			if (p256_ecdsa_verify_ret != 0) {
				p256_ecdsa_verify_ret = P256_INVALID_PUBKEY;
				p256_ecdsa_verify_state = 99;
			} else {
				p256_ecdsa_verify_state = 13;
			}
			break;
		}
		case 13 : {
			result = E_BUSY;
			/* (e, s) = R2 = u2 * Qu */
			if (scalar_mult(p256_ecdsa_verify_e, p256_ecdsa_verify_s, p256_ecdsa_verify_px, p256_ecdsa_verify_py, p256_ecdsa_verify_u2) != E_BUSY) {
				p256_ecdsa_verify_state = 14;
			}
			break;
		}
		case 14 : {
			...
			if (u256_diff0(p256_ecdsa_verify_u1) == 0) {
				p256_ecdsa_verify_state = 15;
			} else {
				p256_ecdsa_verify_state = 16;
			}
			break;
		}
			case 15 : {
				...
				u256_cmov(p256_ecdsa_verify_u1, p256_ecdsa_verify_e, 1);
				/* we don't care about the y coordinate */

				p256_ecdsa_verify_state = 17;
				break;
			}
			case 16 : {
				result = E_BUSY;
				...
				/* (px, py) = R1 = u1 * G */
				if (scalar_mult(p256_ecdsa_verify_px, p256_ecdsa_verify_py, p256_gx, p256_gy, p256_ecdsa_verify_u1) != E_BUSY) {
					/* (u1, u2) = R = R1 + R2 */
					point_add_or_double_leaky(p256_ecdsa_verify_u1, p256_ecdsa_verify_u2, p256_ecdsa_verify_px, p256_ecdsa_verify_py, p256_ecdsa_verify_e, p256_ecdsa_verify_s);
		...
		case 17 : {
			...
			m256_done(p256_ecdsa_verify_u1, &p256_p);

			p256_ecdsa_verify_state = 18;
			break;
		}
		case 18 : {
			result = E_BUSY;
			...
			/* 7. Reduce xR mod n */
			ecdsa_m256_mod_n(p256_ecdsa_verify_u1);

			p256_ecdsa_verify_state = 19;
			break;
		}
		case 19 : {
			result = E_BUSY;
			...
			uint32_t diff = u256_diff(p256_ecdsa_verify_u1, p256_ecdsa_verify_r);
			if (diff == 0) {
				p256_ecdsa_verify_ret = P256_SUCCESS;
				p256_ecdsa_verify_state = 99;
			} else {
				p256_ecdsa_verify_ret = P256_INVALID_SIGNATURE;
				p256_ecdsa_verify_state = 99;
		...
		case 99 : {
			...
			p256_ecdsa_verify_state = 100;
			break;
		}

		default : {
			p256_ecdsa_verify_state = 0;
			result = E_NOT_OK;
			break;
		}
	}
...
int p256_ecdsa_verify_stop(int *ret) {
	unsigned int result = E_NOT_OK;
	if (ret != NULL) {
		if (p256_ecdsa_verify_state == 100) {
			*ret = p256_ecdsa_verify_ret;
			result = E_OK;
		} else {
			*ret = P256_INVALID_STATE;
			result = E_NOT_OK;
...
int p256_ecdsa_verify(const uint8_t sig[64], const uint8_t pub[64], const uint8_t *hash, size_t hlen) {
	int ret = P256_INVALID_STATE;
	if (p256_ecdsa_verify_start(sig, pub, hash, hlen) == E_OK) {
		while (p256_ecdsa_verify_update() == E_BUSY);
		if (p256_ecdsa_verify_stop(&ret) == E_OK) {
		}
	}
	return ret;
}
