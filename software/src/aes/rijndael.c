...
void rijndaelEncrypt_sync(const u32 *rk, int nrounds, const u8 *plaintext, u8 *ciphertext) {
	AES_WORK_STATE result = AES_WORK_STATE_BUSY;
	Aes_Working_Variables working_variables;
	working_variables.state = 0;
	while (result == AES_WORK_STATE_BUSY) {
		result = rijndaelEncrypt_async(rk, nrounds, plaintext, ciphertext, &working_variables);
	}
}

void rijndaelDecrypt_sync(const u32 *rk, int nrounds, const u8 *ciphertext, u8 *plaintext) {
	AES_WORK_STATE result = AES_WORK_STATE_BUSY;
	Aes_Working_Variables working_variables;
	working_variables.state = 0;
	while (result == AES_WORK_STATE_BUSY) {
		result = rijndaelDecrypt_async(rk, nrounds, ciphertext, plaintext, &working_variables);
	}
}

AES_WORK_STATE rijndaelEncrypt_async(const u32 *rk, int nrounds, const u8 *plaintext, u8 *ciphertext, Aes_Working_Variables *working_variables) {
	AES_WORK_STATE result = AES_WORK_STATE_EMPTY;
	if (working_variables != NULL) {
		u32 s0 = working_variables->s0;
		u32 s1 = working_variables->s1;
		u32 s2 = working_variables->s2;
		u32 s3 = working_variables->s3;
		u32 t0 = working_variables->t0;
		u32 t1 = working_variables->t1;
		u32 t2 = working_variables->t2;
		u32 t3 = working_variables->t3;
		int r = working_variables->r;

		result = AES_WORK_STATE_BUSY;

		switch (working_variables->state) {
			case 0 : {
				working_variables->rk_add_value = 0;
				working_variables->state = 1;
				break;
			}
			case 1 : {
				/*
				...
				*/
				r = nrounds >> 1;
				working_variables->state = 2;
				break;
			}
			case 2 : {
				rk += working_variables->rk_add_value;
				t0 = Te0[(s0 >> 24) ] ^ Te1[(s1 >> 16) & 0xff] ^ Te2[(s2 >> 8) & 0xff] ^ Te3[(s3 ) & 0xff] ^ rk[4];
				t1 = Te0[(s1 >> 24) ] ^ Te1[(s2 >> 16) & 0xff] ^ Te2[(s3 >> 8) & 0xff] ^ Te3[(s0 ) & 0xff] ^ rk[5];
				t2 = Te0[(s2 >> 24) ] ^ Te1[(s3 >> 16) & 0xff] ^ Te2[(s0 >> 8) & 0xff] ^ Te3[(s1 ) & 0xff] ^ rk[6];
				t3 = Te0[(s3 >> 24) ] ^ Te1[(s0 >> 16) & 0xff] ^ Te2[(s1 >> 8) & 0xff] ^ Te3[(s2 ) & 0xff] ^ rk[7];
				working_variables->rk_add_value += 8;
				rk += 8;
				if (--r == 0) {
					working_variables->state = 3;
					break;
				}
		...
		working_variables->s0 = s0;
		working_variables->s1 = s1;
		working_variables->s2 = s2;
		working_variables->s3 = s3;
		working_variables->t0 = t0;
		working_variables->t1 = t1;
		working_variables->t2 = t2;
		working_variables->t3 = t3;
		working_variables->r = r;
	}
	return result;
}

AES_WORK_STATE rijndaelDecrypt_async(const u32 *rk, int nrounds, const u8 *ciphertext, u8 *plaintext, Aes_Working_Variables *working_variables) {
	AES_WORK_STATE result = AES_WORK_STATE_EMPTY;
	if (working_variables != NULL) {
		u32 s0 = working_variables->s0;
		u32 s1 = working_variables->s1;
