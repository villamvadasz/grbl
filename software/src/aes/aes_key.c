...
__attribute__(( weak ))void aes_LoadROMKey(unsigned char * rom_key) {
	if (rom_key != NULL) {
		sprintf((char *)rom_key, "1234567890abcdef");
	}
}
...
void init_aes_key(void) {
	...
	memset((char *)aes_KeyHolder[AES_ROM].key, 0x00, 16);
	aes_KeyHolder[AES_ZERO_KEY].challengeResponse = 0;
	aes_KeyHolder[AES_ZERO_KEY].keybits = AES_KEYBITS;
	aes_KeyHolder[AES_ZERO_KEY].nrounds_en = rijndaelSetupEncrypt(aes_KeyHolder[AES_ROM].rk_en, aes_KeyHolder[AES_ROM].key, aes_KeyHolder[AES_ROM].keybits);
	aes_KeyHolder[AES_ZERO_KEY].nrounds_de = rijndaelSetupDecrypt(aes_KeyHolder[AES_ROM].rk_de, aes_KeyHolder[AES_ROM].key, aes_KeyHolder[AES_ROM].keybits);
...
void do_aes_key_1ms(void) {
	if (aes_key_triggerLoadKeyFromEeprom) {
		...
		if (x < AES_MAXKEY) {
			if (x == 0) {
				aes_LoadROMKey((unsigned char *)aes_KeyHolder[AES_ROM].key);
			} else {
