...
unsigned int grbl_report_sys_log_2 = 0;
unsigned int grbl_report_sys_log_3 = 0;
unsigned int grbl_report_sys_log_4 = 0;
...
char grbl_report_failed_line_buffer[GRBL_LINE_BUFFER_SIZE];
...
void init_grbl_report(void) {
	...
	memset(grbl_report_failed_line_buffer, 0, sizeof(grbl_report_failed_line_buffer));
}
...
void do_grbl_report(void) {
	if (do_grbl_report_50ms) {
		...
		if (grbl_report_sys_log_0) {
			switch (grbl_report_sys_log_0_state) {
				case 0 : {
					...
					printPgmString("VERSION_APP_SERIAL_NUMBER: ");
					print_uint32_base16(VERSION_APP_SERIAL_NUMBER);
					printPgmString("\r\n");
					
					printPgmString("GRBL Serial Number: ");
					print_uint32_base16(grbl_serial_number);
					printPgmString("\r\n");
				...
				case 16 : {
					extern uint8 app_sntp_user_called;
					extern uint32 app_dwSNTPSeconds;

					printPgmString("app_sntp_user_called: ");
					print_uint8_base10(app_sntp_user_called);
					printPgmString("\r\n");

					printPgmString("app_dwSNTPSeconds: ");
					print_uint32_base10(app_dwSNTPSeconds);
					printPgmString("\r\n");

					{
						TimeDate ptr;
						rtc_getTime(&ptr);
						printPgmString("UTC Date: ");
						print_uint32_base10((int)ptr.year);
						printPgmString("-");
						print_uint32_base10((int)ptr.month);
						printPgmString("-");
						print_uint32_base10((int)ptr.day);
						printPgmString("\r\n");
						printPgmString("UTC Time: ");
						if (ptr.hour < 10) {
							print_uint32_base10((int)0);
						}
						print_uint32_base10((int)ptr.hour);
						printPgmString(":");
						if (ptr.min < 10) {
							print_uint32_base10((int)0);
						}
						print_uint32_base10((int)ptr.min);
						printPgmString(":");
						if (ptr.sec < 10) {
							print_uint32_base10((int)0);
						}
						print_uint32_base10((int)ptr.sec);
						printPgmString("\r\n");
					}
				...
				case 18 : {
					printPgmString("grbl_report_failed_line_buffer: ");
					if (strlen(grbl_report_failed_line_buffer) != 0) {
						printPgmString(grbl_report_failed_line_buffer);
					}
					printPgmString("\r\n");
					grbl_report_sys_log_0_state++;
		...
		if (grbl_report_sys_log_2) {
			grbl_report_sys_log_2 = 0;
			report_status_message(STATUS_OK, 0);
		}
		if (grbl_report_sys_log_3) {
			grbl_report_sys_log_3 = 0;
			report_status_message(STATUS_OK, 0);
		}
		if (grbl_report_sys_log_4) {
			grbl_report_sys_log_4 = 0;
			report_status_message(STATUS_OK, 0);
		}
	}
}
...
void report_feedback_message(uint8 message_code) {
	...
	switch(message_code) {
		...
		case MESSAGE_ALARM_UNLOCK_FAILED_EMERGENCYSTOP:
			printPgmString(("Failed to clear alarms due to pending problem! Emergency Stop")); break;
		case MESSAGE_ALARM_UNLOCK_FAILED_POWER:
			printPgmString(("Failed to clear alarms due to pending problem! Power")); break;
...
void report_grbl_crypto_0(void) {
	grbl_report_sys_log_2 = 1;
}

void report_grbl_crypto_1(void) {
	grbl_report_sys_log_3 = 1;
}

void report_grbl_crypto_2(void) {
	grbl_report_sys_log_4 = 1;
}
...
void report_build_info(void)
{
  ...
  putChar_grbl(',');
  print_uint32_base10(GRBL_RX_LEN_PROTOCOL);
