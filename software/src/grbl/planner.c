...
uint8 block_buffer_tail;     // Index of the block to process now
uint8 block_buffer_head;     // Index of the next block to be pushed
uint8 next_buffer_head;      // Index of the next buffer head
uint8 block_buffer_planned;  // Index of the optimally planned block
...
uint8 plan_next_block_index(uint8 block_index) {
	block_index++;
...
uint8 plan_prev_block_index(uint8 block_index) {
	if (block_index == 0) {
...
void planner_recalculate(void) {
	...
	uint8 block_index = plan_prev_block_index(block_buffer_head);
...
void plan_discard_current_block(void) {
	if (block_buffer_head != block_buffer_tail) { // Discard non-empty buffer.
		uint8 block_index = plan_next_block_index( block_buffer_tail );
...
uint8 plan_check_full_buffer(void) {
...
void plan_compute_profile_parameters(plan_block_t *block, float nominal_speed, float prev_nominal_speed) {
...
uint8 plan_buffer_line(float *target, plan_line_data_t *pl_data) {
	...
	for (idx=0; idx<N_AXIS; idx++) {
		...
		target_steps[idx] = lround_my(target[idx]*settings.steps_per_mm[idx]);
...
uint8 plan_get_block_buffer_available(void) {
...
uint8 plan_get_block_buffer_count(void) {
	if (block_buffer_head >= block_buffer_tail) {
