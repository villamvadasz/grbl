...
// and timer accuracy.  Do not alter these settings unless you know what you are doing.
	...
	// AMASS_LEVEL0: Normal operation. No AMASS. No upper cutoff frequency. Starts at LEVEL1 cutoff frequency.
		...
		#define AMASS_LEVEL1 ((GetPeripheral3Clock())/8000) // Over-drives ISR (x2). Defined as F_CPU/(Cutoff frequency in Hz)
		#define AMASS_LEVEL2 ((GetPeripheral3Clock())/4000) // Over-drives ISR (x4)
		#define AMASS_LEVEL3 ((GetPeripheral3Clock())/2000) // Over-drives ISR (x8)
	#endif
	#ifdef CPU_MAP_PIC32_MX
		#define AMASS_LEVEL1 ((GetPeripheralClock())/8000) // Over-drives ISR (x2). Defined as F_CPU/(Cutoff frequency in Hz)
		#define AMASS_LEVEL2 ((GetPeripheralClock())/4000) // Over-drives ISR (x4)
		#define AMASS_LEVEL3 ((GetPeripheralClock())/2000) // Over-drives ISR (x8)
	#endif
...
typedef struct _st_block_t{
	uint32 steps[N_AXIS];
	uint32 step_event_count;
	uint8 direction_bits;
	uint8 is_pwm_rate_adjusted; // Tracks motions that require constant laser power/rate
} st_block_t;
...
typedef struct _segment_t{
	uint16 n_step;		   // Number of step events to be executed for this segment
	uint16 cycles_per_tick;  // Step distance traveled per ISR tick, aka step rate.
	uint8  st_block_index;   // Stepper block data index. Uses this information to execute this segment.
	#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
		uint8 amass_level;	// Indicates AMASS level for the ISR to execute this segment
	#else
		uint8 prescaler;	  // Without AMASS, a prescaler is required to adjust for slow timing.
	#endif
	float spindle_pwm;
} segment_t;
...
typedef struct _stepper_t{
	...
	uint32 counter_x;		// Counter variables for the bresenham line tracer
	uint32 counter_y;
	uint32 counter_z;
	#ifdef STEP_PULSE_DELAY
		uint8 step_bits;  // Stores out_bits output to complete the step pulse delay
	#endif

	uint8 execute_step;	 // Flags step execution for each interrupt.
	uint8 step_pulse_time;  // Step pulse reset time after step rise
	uint8 step_outbits;		 // The next stepping-bits to be output
	uint8 dir_outbits;
	#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
		uint32 steps[N_AXIS];
	#endif

	uint16 step_count;	   // Steps remaining in line segment motion
	uint8 exec_block_index; // Tracks the current st_block index. Change indicates new block.
	st_block_t *exec_block;   // Pointer to the block data for the segment being executed
	segment_t *exec_segment;  // Pointer to the segment being executed
} stepper_t;
stepper_t stepper_st;
...
uint8 step_port_invert_mask = 0;
uint8 dir_port_invert_mask = 0;

// Used to avoid ISR nesting of the "Stepper Driver Interrupt". Should never occur though.
volatile uint8 stepper_busy = 0;

uint8 isStepperRunning = 0;
...
typedef struct {
	uint8 st_block_index;  // Index of stepper common data block being prepped
	uint8 recalculate_flag;
	...
	uint8 ramp_type;	  // Current segment ramp state
...
void init_stepper(void) {
	...
	stepper_busy = 0;
	isStepperRunning = 0;
	pl_block = NULL;	 // Pointer to the planner block being prepped
	st_prep_block = NULL;  // Pointer to the stepper block data being prepped
	...
	// Configure step and direction interface pins
		...
		STEP_Z_DDR = 0;
	#endif
	#ifdef DIRECTION_X_DDR
		DIRECTION_X_DDR = 0;
	#endif
	#ifdef DIRECTION_Y_DDR
		DIRECTION_Y_DDR = 0;
	#endif
	#ifdef DIRECTION_Z_DDR
		DIRECTION_Z_DDR = 0;
	#endif

	// Configure Timer 1: Stepper Driver Interrupt
	// Configuration done outside
...
// Stepper state initialization. Cycle should only start if the stepper_st.cycle_start flag is
...
void st_wake_up(void) {
	...
	stepper_st.step_outbits = step_port_invert_mask;

	// Initialize step pulse timing from settings. Here to ensure updating after re-writing.
	#ifdef STEP_PULSE_DELAY
		// Set total step pulse time after direction pin set. Ad hoc computation from oscilloscope.
		stepper_st.step_pulse_time = -(((settings.pulse_microseconds+STEP_PULSE_DELAY-2)*TICKS_PER_MICROSECOND) >> 3);
		// Set delay between direction pin write and step command.
		set_grbl_tmr0_trigger_Step_Pulse_Delay(-(((settings.pulse_microseconds)*TICKS_PER_MICROSECOND) >> 3));
	#else // Normal operation
		// Set step pulse time. Ad hoc computation from oscilloscope. Uses two's complement.
		stepper_st.step_pulse_time = -(((settings.pulse_microseconds-2)*TICKS_PER_MICROSECOND) >> 3);
	#endif

	// Enable Stepper Driver Interrupt
	set_grbl_tmr1_enabled();
}

// Stepper shutdown
void st_go_idle(void) {
	// Disable Stepper Driver Interrupt. Allow Stepper Port Reset Interrupt to finish, if active.
	set_grbl_tmr1_disabled();
	stepper_busy = false;
	...
	#endif
	if (((settings.stepper_idle_lock_time != 0xff) || system_get_exec_alarm()) && sys.state != STATE_HOMING) {
		// Force stepper dwell to lock axes for a defined amount of time to ensure the axes come to a complete
		// stop and not drift from residual inertial forces at the end of the last movement.
		write_timer(&stepper_idle_lock_timer, settings.stepper_idle_lock_time);
		#ifndef STEPPERS_ALWAYS_ENABLED
...
{		
	if (stepper_busy) {//Do not needed on PIC32. Not possible.
		return;
	} // The busy-flag is used to avoid reentering this interrupt
   
	// Set the direction pins a couple of nanoseconds before we step the steppers
	if (stepper_st.dir_outbits & (1<<X_DIRECTION_BIT)) {
		#ifdef DIRECTION_X_PORT
			DIRECTION_X_PORT = 1;
		#endif
	} else {
		#ifdef DIRECTION_X_PORT
	...
	if (stepper_st.dir_outbits & (1<<Y_DIRECTION_BIT)) {
		#ifdef DIRECTION_Y_PORT
			DIRECTION_Y_PORT = 1;
		#endif
	} else {
		#ifdef DIRECTION_Y_PORT
	...
	if (stepper_st.dir_outbits & (1<<Z_DIRECTION_BIT)) {
		#ifdef DIRECTION_Z_PORT
			DIRECTION_Z_PORT = 1;
		#endif
	} else {
		#ifdef DIRECTION_Z_PORT
	...
	// Then pulse the stepping pins
	#ifdef STEP_PULSE_DELAY
		stepper_st.step_bits = stepper_st.step_outbits; // Store out_bits to prevent overwriting.
	#else  // Normal operation
		if (stepper_st.step_outbits & (1<<X_STEP_BIT)) {
			#ifdef STEP_X_PORT
				STEP_X_PORT = 1;
			#endif
		} else {
		...
		if (stepper_st.step_outbits & (1<<Y_STEP_BIT)) {
			#ifdef STEP_Y_PORT
				STEP_Y_PORT = 1;
			#endif
		} else {
		...
		if (stepper_st.step_outbits & (1<<Z_STEP_BIT)) {
			#ifdef STEP_Z_PORT
				STEP_Z_PORT = 1;
			#endif
		} else {
	...
	set_grbl_tmr0_trigger(stepper_st.step_pulse_time);

	stepper_busy = true;
	//No need to have reentrant interrupts since CPU is much more faster than the old one
	//sei(); // Re-enable interrupts to allow Stepper Port Reset Interrupt to fire on-time. 
	// NOTE: The remaining code in this ISR will finish before returning to main program.

	// If there is no step segment, attempt to pop one from the stepper buffer
	if (stepper_st.exec_segment == NULL) {
		// Anything in the buffer? If so, load and initialize next step segment.
		if (segment_buffer_head != segment_buffer_tail) {
			// Initialize new step segment and load number of steps to execute
			stepper_st.exec_segment = &segment_buffer[segment_buffer_tail];

			#ifndef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
				// With AMASS is disabled, set timer prescaler for segments with slow step frequencies (< 250Hz).
				TCCR1B = (TCCR1B & ~(0x07<<CS10)) | (stepper_st.exec_segment->prescaler<<CS10);
			#endif

			// Initialize step segment timing per step and load number of steps to execute.
			set_grbl_tmr1_period(stepper_st.exec_segment->cycles_per_tick);
			//OCR1A = stepper_st.exec_segment->cycles_per_tick;
			stepper_st.step_count = stepper_st.exec_segment->n_step; // NOTE: Can sometimes be zero when moving slow.
			// If the new segment starts a new planner block, initialize stepper variables and counters.
			// NOTE: When the segment data index changes, this indicates a new planner block.
			if ( stepper_st.exec_block_index != stepper_st.exec_segment->st_block_index ) {
				stepper_st.exec_block_index = stepper_st.exec_segment->st_block_index;
				stepper_st.exec_block = &st_block_buffer[stepper_st.exec_block_index];

				// Initialize Bresenham line and distance counters
				stepper_st.counter_x = stepper_st.counter_y = stepper_st.counter_z = (stepper_st.exec_block->step_event_count >> 1);
			}
			stepper_st.dir_outbits = stepper_st.exec_block->direction_bits ^ dir_port_invert_mask;

			#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
				// With AMASS enabled, adjust Bresenham axis increment counters according to AMASS level.
				stepper_st.steps[X_AXIS] = stepper_st.exec_block->steps[X_AXIS] >> stepper_st.exec_segment->amass_level;
				stepper_st.steps[Y_AXIS] = stepper_st.exec_block->steps[Y_AXIS] >> stepper_st.exec_segment->amass_level;
				stepper_st.steps[Z_AXIS] = stepper_st.exec_block->steps[Z_AXIS] >> stepper_st.exec_segment->amass_level;
			#endif

			// Set real-time spindle output as segment is loaded, just prior to the first step.
			if (stepper_st.exec_segment->spindle_pwm == 0.0f) {
				//system_log_spindle_off(0xA); //Overfill log
			} else {
				//system_log_spindle_on(0xB); //Overfill log
			}
			spindle_set_state(SPINDLE_ENABLE_LASTW, stepper_st.exec_segment->spindle_pwm);
			...
			if (stepper_st.exec_block != NULL) {
				if (stepper_st.exec_block->is_pwm_rate_adjusted) { 
	...
	//TODO: Check if move to main routine is feasable.
	// Check probing state.
	...
	// Reset step out bits.
	stepper_st.step_outbits = 0;

	// Execute step displacement profile by Bresenham line algorithm
	#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
		stepper_st.counter_x += stepper_st.steps[X_AXIS];
	#else
		stepper_st.counter_x += stepper_st.exec_block->steps[X_AXIS];
	#endif
	if (stepper_st.counter_x > stepper_st.exec_block->step_event_count) {
		stepper_st.step_outbits |= (1<<X_STEP_BIT);
		stepper_st.counter_x -= stepper_st.exec_block->step_event_count;
		if (stepper_st.exec_block->direction_bits & (1<<X_DIRECTION_BIT)) {
			sys_position[X_AXIS]--; 
		} else { 
			sys_position[X_AXIS]++;
		}
	...
	#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
		stepper_st.counter_y += stepper_st.steps[Y_AXIS];
	#else
		stepper_st.counter_y += stepper_st.exec_block->steps[Y_AXIS];
	#endif
	if (stepper_st.counter_y > stepper_st.exec_block->step_event_count) {
		stepper_st.step_outbits |= (1<<Y_STEP_BIT);
		stepper_st.counter_y -= stepper_st.exec_block->step_event_count;
		if (stepper_st.exec_block->direction_bits & (1<<Y_DIRECTION_BIT)) {
			sys_position[Y_AXIS]--; 
		} else {
			sys_position[Y_AXIS]++;
		}
	...
	#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
		stepper_st.counter_z += stepper_st.steps[Z_AXIS];
	#else
		stepper_st.counter_z += stepper_st.exec_block->steps[Z_AXIS];
	#endif
	if (stepper_st.counter_z > stepper_st.exec_block->step_event_count) {
		stepper_st.step_outbits |= (1<<Z_STEP_BIT);
		stepper_st.counter_z -= stepper_st.exec_block->step_event_count;
		if (stepper_st.exec_block->direction_bits & (1<<Z_DIRECTION_BIT)) {
			sys_position[Z_AXIS]--;
		} else { 
			sys_position[Z_AXIS]++;
		}
	...
	if (sys.state == STATE_HOMING) {
		stepper_st.step_outbits &= sys.homing_axis_lock;
	}

	stepper_st.step_count--; // Decrement step events count
	if (stepper_st.step_count == 0) {
		// Segment is complete. Discard current segment and advance segment indexing.
		stepper_st.exec_segment = NULL;
		if ( ++segment_buffer_tail == SEGMENT_BUFFER_SIZE) {
			segment_buffer_tail = 0;
		}
	}

	stepper_st.step_outbits ^= step_port_invert_mask;  // Apply step port invert mask
...
void TIMER0_OVF_vect(void) {
	...
	MAL_SYNC();
	//Interrupt is disabled outside, no need to disable it here
}
...
void st_reset(void) {
	...
	memset(&prep, 0, sizeof(st_prep_t));
	memset(&stepper_st, 0, sizeof(stepper_t));
	memset(st_block_buffer, 0, sizeof(st_block_buffer));
	memset(segment_buffer, 0, sizeof(segment_buffer));
	...
	stepper_st.dir_outbits = dir_port_invert_mask; // Initialize direction bits to default.
	...
	if (dir_port_invert_mask & (1<<X_DIRECTION_BIT)) {
		#ifdef DIRECTION_X_PORT
			DIRECTION_X_PORT = 1;
		#endif
	} else {
	...
	if (dir_port_invert_mask & (1<<Y_DIRECTION_BIT)) {
		#ifdef DIRECTION_Y_PORT
			DIRECTION_Y_PORT = 1;
		#endif
	} else {
	...
	if (dir_port_invert_mask & (1<<Z_DIRECTION_BIT)) {
		#ifdef DIRECTION_Z_PORT
			DIRECTION_Z_PORT = 1;
		#endif
	} else {
...
uint8 st_next_block_index(uint8 block_index) {
	block_index++;
	...
	while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
		...
		*/
		...
		do {
			...
			if (dt < dt_max) {
				time_var = dt_max - dt;
			} else { // **Incomplete** At ramp junction.
		...
		*/
		...
		uint64_t cycles_precalc = TICKS_PER_MICROSECOND;
		cycles_precalc *= 1000000;
		cycles_precalc *= 60;
		cycles_precalc *= inv_rate;
		uint32 cycles = ceil( cycles_precalc ); // (cycles/step)
...
uint8 st_isRunning(void) {
	return isStepperRunning;
}

uint8 st_isDelayRunning(void) {
	uint8 result = 0;
	if (read_timer(&stepper_idle_lock_timer) != 0) {
		result = 1;
	}
	return result;
}
