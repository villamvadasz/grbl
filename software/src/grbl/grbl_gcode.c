...
parser_state_t gc_state = {0};
parser_block_t gc_block = {0};

float coord_system_eep[N_AXIS];
float coord_offset_eep[N_AXIS];
float grbl_Tool_Length_1 = 0.0f;
float grbl_Tool_Length_2 = 0.0f;

uint8 gc_parser_flags = GC_PARSER_NONE;
// Initialize bitflag tracking variables for axis indices compatible operations.
uint8 axis_words = 0; // XYZ tracking
uint8 axis_command = AXIS_COMMAND_NONE;
uint16 value_words = 0; // Tracks value words.
float block_coord_system[N_AXIS];
// Initialize command and value words and parser flags variables.
uint16 command_words = 0; // Tracks G and M command words. Also used for modal group violations.
uint8 ijk_words = 0; // IJK tracking
uint8 axis_linear = 0;
uint8 coord_select = 0; // Tracks G10 P coordinate selection for execution
uint8 axis_0 = 0;
uint8 axis_1 = 0;



uint8 gc_execute_line_step_1(char *line, int32_t *line_number);
uint8 gc_execute_line_step_2(char *line);
uint8 gc_execute_line_step_3(void);
uint8 gc_execute_line_step_4(int32_t *line_number);
...
void gc_init(void) {
	...
	for (idx=0; idx<N_AXIS; idx++) {
		gc_state.coord_system[idx] = coord_system_eep[idx];
	} 
	for (idx=0; idx<N_AXIS; idx++) {
		gc_state.coord_offset[idx] = coord_offset_eep[idx];
	} 
	...
	if (grbl_Tool_Length_1 == grbl_Tool_Length_2) {
		gc_state.tool_length_offset = grbl_Tool_Length_1;
...
uint8 gc_check_for_sync_needed(char *line) {
	uint8 result = 0;
	...
	while (line[char_counter] != 0) {
		...
		if ( (letter < 'A') || (letter > 'Z') ) {
			...
			if (!read_float(line, &char_counter, &value)) {
				...
				int_value = trunc_my(value);
				//mantissa =  round_my(100 * (value - int_value)); // Compute mantissa for Gxx.x commands.
				if ( (letter == 'M') && (int_value == 30) ) { //M30
					result = true;
					break;
				} else if ( (letter == 'M') && (int_value == 3) ) { //M3
					result = true;
					break;
...
uint8 gc_execute_line(char *line, int32_t *line_number) {
	uint8 result = 0;
	...
	result = gc_execute_line_step_1(line, line_number);
	...
	result = gc_execute_line_step_2(line);
	if (result != STATUS_OK) { 
		int len = strlen(line);
		if (len < GRBL_LINE_BUFFER_SIZE) {
			memcpy(grbl_report_failed_line_buffer, line, len);
			grbl_report_failed_line_buffer[len] = 0;
		}
		return result;
	}
	...
	*/
	result = gc_execute_line_step_4(line_number);
	if (result != STATUS_OK) { return result;}
...
uint8 gc_execute_line_step_1(char *line, int32_t *line_number) {
	{
		gc_parser_flags = GC_PARSER_NONE;
		// Initialize bitflag tracking variables for axis indices compatible operations.
		axis_words = 0; // XYZ tracking
		axis_command = AXIS_COMMAND_NONE;
		value_words = 0; // Tracks value words.
...
uint8 gc_execute_line_step_2(char *line) {
	{
		...
		while (line[char_counter] != 0) { // Loop until no more g-code words in line.
			...
			letter = line[char_counter];
			if((letter < 'A') || (letter > 'Z')) {   // [Expected word letter]
			...
			switch(letter) {
				...
				case 'G': {
					...
					switch(int_value) {
						case 10: case 28: case 30: case 92: {
							...
							if (mantissa == 0) { // Ignore G28.1, G30.1, and G92.1
								if (axis_command) { // [Axis word/command conflict]
						...
						case 43: case 44: case 49: {
							...
							} else if (int_value == 43) {
								if (mantissa == 10) { // G43.1
									gc_block.modal.tool_length = TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC;
								} else {  // [Unsupported G43.x command]
									return STATUS_GCODE_UNSUPPORTED_COMMAND;
								}
							} else if (int_value == 44) {
								if (mantissa == 10) { // G44.1
									gc_block.modal.tool_length = TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC_NEG;
								} else {  // [Unsupported G43.x command]
									return STATUS_GCODE_UNSUPPORTED_COMMAND;
								}
							}
					...
					if (mantissa > 0) { // [Unsupported or invalid Gxx.x command]
				...
				case 'M': {
					...
					if (mantissa > 0) { // [No Mxx.x commands]
						return STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER;
					}
					switch(int_value) {
						...
						case 6 : {//T1 M6
							uint16 toolNumber = gc_block.values.t;
							setToolChangeRunning(toolNumber, 1);
							//TODO Implement here Tool change procedure
							//N191 M06 T3 G43 H3
							break;
						}
						default: {
				...
				default: {
					...
					switch(letter) {
						...
						case 'N': word_bit = WORD_N; gc_block.values.n = trunc_my(value); break;
						case 'P': word_bit = WORD_P; gc_block.values.p = value; break;
						// NOTE: For certain commands, P value must be an integer, but none of these commands are supported.
						// case 'Q': // Not supported
						case 'R': word_bit = WORD_R; gc_block.values.r = value; break;
						case 'S': word_bit = WORD_S; gc_block.values.s = value; break;
						case 'T': word_bit = WORD_T; //T1 M6
					...
					if ( bit(word_bit) & (bit(WORD_F)|bit(WORD_N)|bit(WORD_P)|bit(WORD_T)|bit(WORD_S)) ) {
						if (value < 0.0) { // [Word value cannot be negative]
...
uint8 gc_execute_line_step_3(void) {
	{
		...
		*/
		...
		if (axis_words) {
			if (!axis_command) {// Assign implicit motion-mode
				axis_command = AXIS_COMMAND_MOTION_MODE;
			} 
		}
		...
		if ( bit_istrue(command_words,bit(MODAL_GROUP_G12)) ) { // Check if called in block
			if (gc_block.modal.coord_select > N_COORDINATE_SYSTEM) { // [Greater than N sys]
		...
		switch (gc_block.non_modal_command) {
			case NON_MODAL_SET_COORDINATE_DATA:
				...
				if (bit_isfalse(value_words,((1<<WORD_P)|(1<<WORD_L)))) { // [P/L word missing]
					return STATUS_GCODE_VALUE_WORD_MISSING;
				}
				coord_select = trunc_my(gc_block.values.p); // Convert p value to int.
				if (coord_select > N_COORDINATE_SYSTEM_VAR) {// [Greater than N sys]
					return STATUS_GCODE_UNSUPPORTED_COORD_SYS;
				}
				if (gc_block.values.l != 20) {
					if (gc_block.values.l == 2) { // [G10 L2 R not supported]
				...
				if (coord_select > 0) { // Adjust P1-P6 index to EEPROM coordinate data indexing.
					coord_select--; 
				} else { // Index P0 as the active coordinate system
					coord_select = gc_block.modal.coord_select;
				}
		  
				// NOTE: Store parameter data in IJK values. By rule, they are not in use with this command.
				if (!settings_read_coord_data(coord_select,gc_block.values.ijk)) { // [EEPROM read fail]
					return STATUS_SETTING_READ_FAIL;
				}
		...
		if (gc_block.modal.motion == MOTION_MODE_NONE) {
			...
			if (gc_block.values.f == 0.0) { // [Feed rate undefined]
				return STATUS_GCODE_UNDEFINED_FEED_RATE;
			}
			...
			switch (gc_block.modal.motion) {
				...
				case MOTION_MODE_CCW_ARC:
					...
					if (!(axis_words & (bit(axis_0)|bit(axis_1)))) { // [No axis words in plane]
					...
					if (value_words & bit(WORD_R)) { // Arc Radius Mode
						...
						#else
							if (isequal_position_vector(gc_state.position[X_AXIS], gc_state.position[Y_AXIS], gc_state.position[Z_AXIS], gc_state.position[A_AXIS], gc_block.values.xyz[X_AXIS], gc_block.values.xyz[Y_AXIS], gc_block.values.xyz[Z_AXIS], gc_block.values.xyz[A_AXIS])) { // [Invalid target]
								return STATUS_GCODE_INVALID_TARGET;
							}
						#endif
						...
						*/
						...
						if (h_x2_div_d < 0) { // [Arc radius error]
						...
						*/
						...
						if (delta_r > 0.005) {
							if (delta_r > 0.5) { // [Arc definition error] > 0.5mm
								//return STATUS_GCODE_INVALID_TARGET;  //Hit here, see what happens if not here
							}
							if (delta_r > (0.001*gc_block.values.r)) { // [Arc definition error] > 0.005mm AND 0.1% radius
								//FAIL(STATUS_GCODE_INVALID_TARGET); //Hit here, see what happens if not here
							}
						}
					}
					break;
...
uint8 gc_execute_line_step_4(int32_t *line_number) {
	{
		...
		// protocol_exec_rt_system() when jogging completes or is canceled.
		if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
			...
			uint8 status = jog_execute(&plan_data, &gc_block);
			if (status == STATUS_OK) {
		...
		gc_state.line_number = gc_block.values.n;
			...
			pl_data->line_number = gc_state.line_number; // Record data for planner use.
				if (line_number != NULL) {
					*line_number = gc_state.line_number;
				}
		#endif
		...
		if ((gc_state.spindle_speed != gc_block.values.s) || bit_istrue(gc_parser_flags,GC_PARSER_LASER_FORCE_SYNC)) {
			if (gc_state.modal.spindle != SPINDLE_DISABLE) { 
				if (bit_isfalse(gc_parser_flags,GC_PARSER_LASER_ISMOTION)) {
					if (bit_istrue(gc_parser_flags,GC_PARSER_LASER_DISABLE)) {
						system_log_spindle_off(40);
						spindle_set_state(gc_state.modal.spindle, 0.0);
					} else { 
		...
		// TODO implement here Tool change on tool holder
		gc_state.tool_requested = gc_block.values.t;
		...
		if (gc_state.modal.spindle != gc_block.modal.spindle) {
			...
			if ((pl_data->spindle_speed == 0.0f) || (gc_block.modal.spindle == SPINDLE_DISABLE)) {
				system_log_spindle_off(43);
			} else {
				system_log_spindle_on(0x2C);
			}
			spindle_set_state(gc_block.modal.spindle, pl_data->spindle_speed);
		...
		if (axis_command == AXIS_COMMAND_TOOL_LENGTH_OFFSET ) { // Indicates a change.
			...
			} else if (gc_state.modal.tool_length == TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC) {
				if ( gc_state.tool_length_offset != gc_block.values.xyz[TOOL_LENGTH_OFFSET_AXIS] ) {
					gc_state.tool_length_offset = gc_block.values.xyz[TOOL_LENGTH_OFFSET_AXIS];
					grbl_Tool_Length_1 = gc_state.tool_length_offset;
					grbl_Tool_Length_2 = gc_state.tool_length_offset;
					grbl_eeprom_trigger();
					system_flag_wco_change();
				}
			} else if (gc_state.modal.tool_length == TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC_NEG) {
				if ( gc_state.tool_length_offset != -gc_block.values.xyz[TOOL_LENGTH_OFFSET_AXIS] ) {
					gc_state.tool_length_offset = -gc_block.values.xyz[TOOL_LENGTH_OFFSET_AXIS];
					grbl_Tool_Length_1 = gc_state.tool_length_offset;
					grbl_Tool_Length_2 = gc_state.tool_length_offset;
		...
		if (gc_state.modal.motion != MOTION_MODE_NONE) {
			if (axis_command == AXIS_COMMAND_MOTION_MODE) {
				uint8 gc_update_pos = GC_UPDATE_POS_TARGET;
				uint8 skipUpdateDueToProbe = 0;
				if (gc_state.modal.motion == MOTION_MODE_LINEAR) {
					...
					#endif
					mc_probe_cycle(gc_block.values.xyz, pl_data, gc_parser_flags);
					skipUpdateDueToProbe = 1;
				}

				if (skipUpdateDueToProbe == 0) {
...
void gcode_probe_update_pos(void) {
	memcpy(gc_state.position, gc_block.values.xyz, sizeof(gc_block.values.xyz)); // gc_state.position[] = gc_block.values.xyz[]
}
