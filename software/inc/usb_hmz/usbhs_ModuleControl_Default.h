...
/* Function :  USBHS_IsBDevice_Default
  ...
  Description:
    This template implements the Default variant of the PLIB_USBHS_IsBDevice function.
*/

PLIB_TEMPLATE bool USBHS_IsBDevice_Default( USBHS_MODULE_ID index )
{
    volatile uint8_t * usbBaseAddress = (uint8_t *)_USBHS_BASE_ADDRESS_VREG(index);
    if((*(usbBaseAddress + 0x60)) & 0x80)
    {
        /* This means the B Device mode is set */
        return true;
    }
    else
    {
...
PLIB_TEMPLATE void USBHS_DeviceAttach_Default( USBHS_MODULE_ID index, uint32_t speed )
{
    ...
    {
        /* Enable high speed operation and attach the device */
