...
#if (BLOCK_BUFFER_SIZE > 128)
	#error BLOCK_BUFFER_SIZE is too big.
#endif
...
typedef struct {
  ...
  uint32 steps[N_AXIS];    // Step count along each axis
  uint32 step_event_count; // The maximum step axis count and number of steps required to complete this block.
...
uint8 plan_get_block_buffer_available();

// Returns the number of active blocks are in the planner buffer.
// NOTE: Deprecated. Not used unless classic status reports are enabled in config.h
uint8 plan_get_block_buffer_count();

// Returns the status of the block ring buffer. True, if buffer is full.
uint8 plan_check_full_buffer();

void plan_get_planner_mpos(float *target);

extern void init_planner(void);
