					...
					if (cycle_mask_asynch & bit(idx)) {
						...
						if (skipCalculation == 0) {
							switch (idx) {
								case X_AXIS : {
									if ( bit_istrue(settings.homing_dir_mask, bit(idx)) ) {
										set_axis_position = lround_my((settings.max_travel[X_AXIS] + settings.homing_pulloff) * settings.steps_per_mm[idx]);
									} else {
										set_axis_position = lround_my((-settings.max_travel[X_AXIS] - settings.homing_pulloff) * settings.steps_per_mm[idx]);
									}
									break;
								...
								case Y_AXIS : {
									if ( bit_istrue(settings.homing_dir_mask, bit(idx)) ) {
										set_axis_position = lround_my((settings.max_travel[Y_AXIS] + settings.homing_pulloff) * settings.steps_per_mm[idx]);
									} else {
										set_axis_position = lround_my((-settings.max_travel[Y_AXIS] - settings.homing_pulloff) * settings.steps_per_mm[idx]);
									}
									break;
								...
								case Z_AXIS : {
									if ( bit_istrue(settings.homing_dir_mask, bit(idx)) ) {
										set_axis_position = lround_my((settings.max_travel[Z_AXIS] + settings.homing_pulloff) * settings.steps_per_mm[idx]);
									} else {
										set_axis_position = lround_my((-settings.max_travel[Z_AXIS] - settings.homing_pulloff) * settings.steps_per_mm[idx]);
									}
									break;
