...
void st_go_idle(void) {
	...
	if (((settings.stepper_idle_lock_time != 0xff) || system_get_exec_alarm()) && sys.state != STATE_HOMING) {
		...
		setDelayRunning(settings.stepper_idle_lock_time);
		#ifndef STEPPERS_ALWAYS_ENABLED
...
{		
	...
	set_grbl_tmr0_trigger(st.step_pulse_time);
	...
	if (st.exec_segment == NULL) {
		...
		if (segment_buffer_head != segment_buffer_tail) {
			...
			set_grbl_tmr1_period(st.exec_segment->cycles_per_tick);
			...
			if (st.exec_segment->spindle_pwm == 0.0f) {
				//system_log_spindle_off(0xA); //Overfill log
			} else {
				//system_log_spindle_on(0xB); //Overfill log
			}
			spindle_set_state(SPINDLE_ENABLE_LASTW, st.exec_segment->spindle_pwm);
...
static uint8 st_next_block_index(uint8 block_index) {
	block_index++;
