...
uint8 serialCharReceived = 0;
uint8 protocol_execute_line_status = 0;
...
static uint8 protocol_execute_line(char *line); // Executes an internal system command, defined as a string starting with a '$'
...
void do_protocol_main_loop(void) {
	{
		...
		if (ch != -1) {
			uint8 dataCh = ch;
			protocol_set_serialCharReceived();
...
void protocol_set_serialCharReceived(void) {
	serialCharReceived = 1;
}

uint8 protocol_get_serialCharReceived(void) {
	return serialCharReceived;
}

void protocol_clear_serialCharReceived(void) {
	serialCharReceived = 0;
}
...
static uint8 process_single_char_command(uint8 data) {
	...
	switch (data) {
		...
		case CMD_RESET : {
			...
			//Do nothing. Reset is not supported any more
			result = 1;
			break; // Call motion control reset routine.
		}
		case CMD_BOOTLOADER : {
			grbl_bootloader_trigger();
			result = 1;
			break; // Call motion control reset routine.
		}
	...
	if (line_flags) {
		...
		if ((c == '\n') || (c == '\r')) { // End of line reached
			...
			if (line[0] == 0) {
				...
				ethernet_set_mac_address(&line[4]);
				memset(line, 0, LINE_BUFFER_SIZE);
			} else if (line[0] == '$') {
				// Grbl '$' system command
				result = 1;
...
uint8 protocol_execute_line(char *line) {
	uint8 char_counter = 1;
	uint8 helper_var = 0; // Helper variable
