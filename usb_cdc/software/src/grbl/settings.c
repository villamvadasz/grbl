...
void settings_write_coord_data(uint8 coord_select, float *coord_data) {
	...
	grbl_eeprom_trigger();
	//uint32 addr = coord_select*(sizeof(float)*N_AXIS+1) + EEPROM_ADDR_PARAMETERS;
...
uint8 settings_read_build_info(char *line) {
	memcpy(line, eep_build_info, LINE_BUFFER_SIZE);
	return true;
}
...
uint8 read_global_settings(void) {
	...
	uint8 version = eep_settings_version;
  ...
  if (parameter >= AXIS_SETTINGS_START_VAL) {
    ...
    switch(parameter) {
      ...
      case 4: // Reset to ensure change. Immediate re-init may cause problems.
        if (int_value) { settings.flags |= BITFLAG_EMPTY; }
        else { settings.flags &= ~BITFLAG_EMPTY; }
        break;
      ...
      case 32:
        ...
        break;
      case 33: settings.grbl_parameter_automatic_tool_change = value; break;
      default:
